...
/**
 * Common testing code for classes that implement heaps of ints with int-valued
 * priorities.
 */
public abstract class SharedTestHelpersIntHeap {
	
	private final IntFunction<IntPriorityQueue> minHeapFactory;
	private final IntFunction<IntPriorityQueue> maxHeapFactory;
	private final CaseDataCreator cdc;
	
	SharedTestHelpersIntHeap(IntFunction<IntPriorityQueue> minHeapFactory, IntFunction<IntPriorityQueue> maxHeapFactory) {
	...
	final void createMaxHeap() {
		...
		IllegalArgumentException thrown = assertThrows( 
			IllegalArgumentException.class,
			() -> maxHeapFactory.apply(0)
		);
	}
	
	final void clearMinHeap() {
		IntPriorityQueue pq = minHeapFactory.apply(10);
		for (int i = 0; i < 5; i++) {
			pq.offer(i, 42*i);
		}
	...
	final void clearMaxHeap() {
		IntPriorityQueue pq = maxHeapFactory.apply(10);
		for (int i = 0; i < 5; i++) {
			pq.offer(i, 42*i);
		}
		assertEquals(5, pq.size());
	...
	final void randomPriorityMinHeap() {
		int n = 31;
		int[] e = cdc.createElements(n);
		int[] p = cdc.shuffle(cdc.orderedArray(n));
		IntPriorityQueue pq = minHeapFactory.apply(n);
		assertEquals(0, pq.size());
		assertTrue(pq.isEmpty());
		assertEquals(n, pq.domain());
	...
	final void copy(boolean pollReturnsNegativeOneWhenEmpty) {
		IntPriorityQueue min0 = minHeapFactory.apply(10);
		IntPriorityQueue max0 = maxHeapFactory.apply(10);
		IntPriorityQueue min1 = minHeapFactory.apply(10);
		for (int i = 0; i < 5; i++) {
			min1.offer(i, 42*i);
		}
		IntPriorityQueue min2 = minHeapFactory.apply(10);
		for (int i = 0; i < 5; i++) {
			min2.offer(i, 43*i);
		}
		IntPriorityQueue max1 = maxHeapFactory.apply(10);
		for (int i = 0; i < 5; i++) {
			max1.offer(i, 42*i);
		}
		IntPriorityQueue max2 = maxHeapFactory.apply(10);
		for (int i = 0; i < 5; i++) {
			max2.offer(i, 43*i);
		}
		IntPriorityQueue minMultiLevel = minHeapFactory.apply(24);
		IntPriorityQueue maxMultiLevel = maxHeapFactory.apply(24);
		...
		IntPriorityQueue minMultiLevelCopy = (IntPriorityQueue)((Copyable)minMultiLevel).copy();
		@SuppressWarnings("unchecked")
		IntPriorityQueue maxMultiLevelCopy = (IntPriorityQueue)((Copyable)maxMultiLevel).copy();
		int lastMin = -1000;
		int lastMax = 1000;
		for (int i = 0; i < 20; i++) {
			...
			int minP = minMultiLevel.peekPriority();
			int maxP = maxMultiLevel.peekPriority();
		...
		@SuppressWarnings("unchecked")
		IntPriorityQueue minCopy0 = (IntPriorityQueue)((Copyable)min0).copy();
		@SuppressWarnings("unchecked")
		IntPriorityQueue minCopy1 = (IntPriorityQueue)((Copyable)min1).copy();
		@SuppressWarnings("unchecked")
		IntPriorityQueue minCopy2 = (IntPriorityQueue)((Copyable)min2).copy();
		@SuppressWarnings("unchecked")
		IntPriorityQueue maxCopy0 = (IntPriorityQueue)((Copyable)max0).copy();
		@SuppressWarnings("unchecked")
		IntPriorityQueue maxCopy1 = (IntPriorityQueue)((Copyable)max1).copy();
		@SuppressWarnings("unchecked")
		IntPriorityQueue maxCopy2 = (IntPriorityQueue)((Copyable)max2).copy();
