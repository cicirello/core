...
/**
 * Common testing code for classes that implement heaps of ints with double-valued
 * priorities.
 */
public abstract class SharedTestHelpersIntHeapDouble {
	...
	SharedTestHelpersIntHeapDouble(IntFunction<IntPriorityQueueDouble> minHeapFactory, IntFunction<IntPriorityQueueDouble> maxHeapFactory) {
	...
	final void copy(boolean pollReturnsNegativeOneWhenEmpty) {
		...
		IntPriorityQueueDouble minMultiLevelCopy = (IntPriorityQueueDouble)((Copyable)minMultiLevel).copy();
		@SuppressWarnings("unchecked")
		IntPriorityQueueDouble maxMultiLevelCopy = (IntPriorityQueueDouble)((Copyable)maxMultiLevel).copy();
		double lastMin = -1000;
		double lastMax = 1000;
		for (int i = 0; i < 20; i++) {
