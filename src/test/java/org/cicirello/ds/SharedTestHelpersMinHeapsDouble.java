 ...
 * specifically for the min heap cases with double-valued priorities.
 */
public abstract class SharedTestHelpersMinHeapsDouble extends SharedTestHelpersHeapsDouble {
	...
	SharedTestHelpersMinHeapsDouble(Supplier<PriorityQueueDouble<String>> factory, Function<Collection<PriorityQueueNode.Double<String>>, PriorityQueueDouble<String>> fromListFactory) {
		super(true);
	...
	final void defaultMinHeap() {
		...
		assertNull(pq.peek());
		assertEquals(Double.POSITIVE_INFINITY, pq.peekPriority());
		for (int i = 0; i < n; i++) {
		...
		assertEquals(Double.POSITIVE_INFINITY, pq.peekPriority("hello"));
		for (int i = 0; i < n; i++) {
