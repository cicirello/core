 ...
 * distinct integers in the interval [0, n), and with priority values of type int.
 *
 ...
 *
 * <p><b>Priority order:</b> IntFibonacciHeap instances are created via factory methods with names
 * beginning with <code>create</code>. The priority order depends upon the factory method used to
 * create the IntFibonacciHeap. Methods named <code>createMinHeap</code> produce a min heap with
 ...
 * empty.
 *
 * <p><b>Purpose:</b> The purpose of such an IntFibonacciHeap is to support implementations of
 ...
 *
 * <p>For a more general purpose binary heap, see the {@link FibonacciHeap} class.
 *
 ...
 *       #createMinHeap(int)}, {@link #domain()}, {@link #isEmpty()}, {@link #offer(int, int)},
 *       {@link #peek()}, {@link #peekPriority()}, {@link #peekPriority(int)}, {@link
 *       #promote(int,int)}, {@link #size()}
 *   <li><b>O(lg n):</b> {@link #demote(int,int)}, {@link #poll()}
 *   <li><b>O(n):</b> {@link #clear()}, {@link #copy()}
 * </ul>
 *
 * <p>The amortized runtime of {@link #change(int,int)} depends on the direction of change. If the
 * priority is decreased for a min-heap or increased for a max-heap, the amortized runtime of {@link
 * #change(int,int)} is O(1); but if the priority is increased for a min-heap or decreased for a
 * max-heap, then the amortized time of {@link #change(int,int)} is O(lg n).
 *
