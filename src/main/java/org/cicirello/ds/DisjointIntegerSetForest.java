 ...
 * Set Forests are described by Cormen, Leiserson, Rivest, and Stein describe in <i>Introduction to
 * Algorithms</i>, including the use of the union by rank heuristic and path compression.
 *
 * <p>For disjoint sets of objects, see the {@link DisjointSetForest} class.
 *
...
public final class DisjointIntegerSetForest implements Copyable<DisjointIntegerSetForest> {
  ...
  /**
   * Initializes a disjoint set forest containing the integers from 0 through n-1, each initially in
   * a set by itself.
   *
  ...
  /**
   * Checks if this DisjointIntegerSetForest is equal to another. This test of equality is true if
   * and only if the two DisjointIntegerSetForest are structurally the same (i.e., not just same set
   * of disjoint sets, but also exact same internal structure).
   *
   * @param other The other DisjointIntegerSetForest.
   * @return true if and only if two DisjointIntegerSetForest instances are structurally identical.
   */
  @Override
  public boolean equals(Object other) {
    if (other instanceof DisjointIntegerSetForest casted) {
  ...
  /**
   * Finds the id of the set currently containing the element. Calls to this method also apply path
   * compression to make subsequent calls faster.
   *
   * @param element The element whose set is sought, which must be in [0,n).
   * @return the id of the set that currently contains the element, where the id is a representative
   *     member of that set.
   * @throws ArrayIndexOutOfBoundsException if element is less than 0 or if element is greater than
   *     or equal to n.
   */
   ...
   *     set forest.
   * @throws ArrayIndexOutOfBoundsException if element1 or element2 is less than 0 or if element1 or
   *     element2 is greater than or equal to n.
   */
  ...
  /**
   * Merges two sets into one, where the sets are chosen by specifying a representative member of
   * each. If the two elements are already in the same set, then no merger takes place. Calls to
   * this method also apply path compression while finding the sets containing each of the elements,
   * and the union itself uses the union by rank heuristic. The combination of these two should
   * improve cost of subsequent operations.
   *
