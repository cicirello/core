...
/**
 * An implementation of a Fibonacci Heap of (element, priority) pairs, such that the elements are
 ...
 *
 * <p><b>Priority order:</b> IntFibonacciHeapDouble instances are created via factory methods with
 * names beginning with <code>create</code>. The priority order depends upon the factory method used
 * to create the IntFibonacciHeapDouble. Methods named <code>createMinHeap</code> produce a min heap
 ...
 * example an IntFibonacciHeapDouble with an element domain of [0,100) is created:
 *
 * <pre><code>
 * IntFibonacciHeapDouble pq = IntFibonacciHeapDouble.createMinHeap(100);
 * </code></pre>
 *
 * <p>In the above example, the element domain is [0,100) and the IntFibonacciHeapDouble is
 * initially empty.
 *
 * <p><b>Purpose:</b> The purpose of such an IntFibonacciHeapDouble is to support implementations of
 ...
 *
 * <p>For a more general purpose binary heap, see the {@link FibonacciHeapDouble} class.
 *
 * <p><b>Method runtimes:</b> The asymptotic runtime of the methods of this class are as follows
 * (where n is the current size of the heap). Note that in many cases in this list, the runtimes are
 * amortized time and not actual time (see a reference on Fibonacci heaps for details).
 *
 * <ul>
 ...
 * the priority is decreased for a min-heap or increased for a max-heap, the amortized runtime of
 * {@link #change(int,double)} is O(1); but if the priority is increased for a min-heap or decreased
 * for a max-heap, then the amortized time of {@link #change(int,double)} is O(lg n).
 *
...
public final class IntFibonacciHeapDouble
   ...
   *
   * @throws IndexOutOfBoundsException if element is negative, or if element is greater than or
   *     equal to the domain n. * @throws NullPointerException if element is not in the heap
   */
  @Override
