...
/**
 * An implementation of a Binary Heap. An instance of a BinaryHeap contains (element, priority)
 * pairs, such that the elements are distinct. The priority values are of type int.
 *
 * <p><b>Priority order:</b> BinaryHeap instances are created via factory methods with names
 * beginning with <code>create</code>. The priority order depends upon the factory method used to
 * create the BinaryHeap. Methods named <code>createMinHeap</code> produce a min heap with priority
 * order minimum-priority-first-out. Methods named <code>createMaxHeap</code> produce a max heap
 ...
 *   <li><b>O(n + m):</b> {@link #addAll(Collection)}, {@link #merge(BinaryHeap)}, {@link
...
public final class BinaryHeap<E>
  ...
  private BinaryHeap(int initialCapacity, PriorityComparator compare) {
    ...
    extreme =
        compare.belongsAbove(0, 1) ? java.lang.Integer.MAX_VALUE : java.lang.Integer.MIN_VALUE;
  }
  ...
  /**
   * Creates an empty BinaryHeap with the {@link #DEFAULT_INITIAL_CAPACITY} as the initial capacity,
   * and a minimum-priority-first-out priority order.
   *
  ...
  /**
   * Creates an empty BinaryHeap with a specified initial capacity, and a minimum-priority-first-out
   * priority order.
   *
   * @param initialCapacity The initial capacity, which must be positive.
   * @param <E> The type of elements contained in the BinaryHeap.
  ...
  /**
   * Creates a BinaryHeap from a collection of (element, priority) pairs, with a
  ...
  /**
   * Creates an empty BinaryHeap with the {@link #DEFAULT_INITIAL_CAPACITY} as the initial capacity,
   * and a maximum-priority-first-out priority order.
   *
   * @param <E> The type of elements contained in the BinaryHeap.
  ...
  /**
   * Creates an empty BinaryHeap with a specified initial capacity, and a maximum-priority-first-out
   * priority order.
   *
   * @param initialCapacity The initial capacity, which must be positive.
   * @param <E> The type of elements contained in the BinaryHeap.
  ...
  /**
   * Checks if this BinaryHeap contains the same (element, priority) pairs as another BinaryHeap,
   * including the specific order within the BinaryHeap, as well as that the priority order is the
   * same.
   *
  ...
  /**
   * Decreases the capacity to the current size of the BinaryHeap, provided size is at least 1, and
   * otherwise decreases capacity to 1. If the size and the capacity are the same, then this method
   * does nothing.
   */
