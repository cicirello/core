...
public final class DoubleList implements Copyable<DoubleList> {
  ...
  /**
   * Initializes an empty DoubleList, with a specified initial capacity. This may be useful if you
   * anticipate adding a large number of elements as it can help minimize the number of array
   * reallocations.
   *
   ...
   *
   * @param initialContents The initial contents of the list, which must contain at least one
   *     element. This constructor clones this array so it is free of unintended side-effects.
  ...
  public DoubleList(double[] initialContents) {
    if (initialContents.length < 1) {
      throw new IllegalArgumentException(
          "Initialization from array requires at least one element in array.");
    }
  ...
  /**
   * Increases capacity if necessary to achieve a specified minimum capacity. If increased, the new
   * capacity may or may not exactly equal to requested minimum. The only guarantee is that it is at
   ...
   * @return The index of the first occurrence of the element in the list from the left, or -1 if
   ...
   *
   * @param element The element searched for.
   * @return The index of the last occurrence of the element in the list, or -1 if the element
   *     doesn't exist.
   */
  ...
  /**
   * Returns an array containing all of the element from the list. This method constructs a new
   * array with length equal to the current size() of the list. This method does not expose the
   * encapsulated array.
   *
  ...
  /**
   * Reduces the capacity of the list to the current value of size(). However, if size() is 0, this
   * method will trim the list capacity to 1. If you don't anticipate adding additional elements to
   * the list, and if your application is memory constrained, then this method can save memory.
   */
  ...
  /**
   * Checks if this list is equal to another list, such that they are the same size, and contain all
   * of the same elements in the same order.
   *
   * @param other The other list
   * @return true If the lists are the same size and contain the same elements in the same order.
   */
  @Override
  public boolean equals(Object other) {
    if (other instanceof DoubleList o) {
      if (size != o.size) {
        return false;
      }
      for (int i = 0; i < size; i++) {
