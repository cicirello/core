 ...
 *
 * <p>For a strongly encapsulated partially-filled array of primitive doubles, including
 * growability, see the {@link DoubleList} class.
 *
...
public final class DoubleArray {
  ...
  /**
   * Initializes an empty DoubleArray, with a specified length for the internal array. By empty, we
   * mean size() equals 0.
   *
   * @param length the length of the array within the DoubleArray
  ...
  /**
   * Initializes an DoubleArray so as to wrap an existing array. A reference to this array is stored
   * in the DoubleArray object, and thus changes to the array externally are observable within the
   * DoubleArray; and likewise changes via the methods of this class are observable directly via the
   * array reference.
   *
   * <p>The size() of this DoubleArray is 0, and doesn't depend upon the length of array. However,
   * the length() of this DoubleArray equals array.length.
   *
  ...
  /**
   * Checks if this DoubleArray is equal to another DoubleArray, such that they wrap the same array
   * instance and are of the same size().
   *
   * @param other The other DoubleArray
  ...
  public int hashCode() {
    int h = 0;
    for (int i = 0; i < size; i++) {
      h = (31 * h) + Double.hashCode(array[i]);
    }
    return h;
  }
